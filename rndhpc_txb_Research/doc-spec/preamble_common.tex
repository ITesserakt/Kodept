...
%----------------------------------------------------------
% дополнительные определения (работать будет только при подключенном пакете listings)

% Подписи к листингам на русском языке.
\renewcommand\lstlistlistingname{\cyr\CYRL\cyri\cyrs\cyrt\cyri\cyrn\cyrg\cyri}

\lstloadlanguages{[LaTeX]TeX, bash, XML, make, C++}

\lstset
{
		language=C++,
		extendedchars=\true, % включаем не латиницу (работает только если указано \true, а не true)
		frame=tb, % рамка сверху и снизу
%		escapechar=|, % |«выпадаем» в LATEX|
		xleftmargin=0.5cm,
		xrightmargin=0.5cm,
...
%		aboveskip=5pt,
		...
		showspaces=false,
		showstringspaces=false,
		breakatwhitespace=true,         % sets if automatic breaks should only happen at whitespace (работает только если указано true, а не \true)
		breaklines=true,                 % sets automatic line breaking (работает только если указано true, а не \true)
		basicstyle=\color{black}\small\sffamily,%\ttfamily,% \sffamily
		commentstyle=\color{gray}\itshape, % шрифт для комментариев
		stringstyle=\color{orange},
%		stringstyle=\bfseries, % шрифт для строк
...
\colorbox{black!5}{\parbox{15cm}{{\large\textbf{``}}\textit{#1}}}\linebreak
%\vspace{-5pt}
\parbox{15cm}{\begin{flushright}#2\end{flushright}}
}
%----------------------------------------------------------
