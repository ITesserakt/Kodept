...
impl<'a, T, C, E> FusedIterator for DagChildIter<'a, T, C, E>
where
    C: SlotMapContainer<Key = CommonKey, Data = Node<T, E>>,
    E: 'a,
{
...
pub struct DagChildIdIter<'a, T, C, E = ()>
where
    C: SlotMapContainer<Key = CommonKey, Data = Node<T, E>>,
{
    inner: &'a DagImpl<T, C>,
...
impl<'a, T, C, E> DagChildIdIter<'a, T, C, E>
where
    C: SlotMapContainer<Key = CommonKey, Data = Node<T, E>>,
{
    pub(crate) fn new(dag: &'a DagImpl<T, C>, parent_id: NodeKey) -> Self {
        let children = match parent_id {
            NodeKey::Root => dag.root_children.as_ref(),
            NodeKey::Child(id) => dag.arena[id].children.as_ref(),
        }
...
impl<'a, T, C, E> Iterator for DagChildIdIter<'a, T, C, E>
where
    C: SlotMapContainer<Key = CommonKey, Data = Node<T, E>>,
    E: 'a,
{
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        Some(NodeKey::Child(id))
    }
}

impl<'a, T, C, E> FusedIterator for DagChildIdIter<'a, T, C, E>
where
    C: SlotMapContainer<Key = CommonKey, Data = Node<T, E>>,
    E: 'a,
{
