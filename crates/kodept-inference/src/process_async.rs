...
#[derive(Debug)]
struct PartialInfer<Name> {
    assumptions: AssumptionSet<Name>,
    constraints: SmallVec<[Constraint; CONSTRAINTS_SIZE]>,
    current_type: Interned<MonomorphicType>,
}
...
where
    Name: Hash + Eq,
{
    fn new(current_type: impl InternInto<MonomorphicType>) -> Self {
        Self {
    ...
    fn with_assumption(mut self, key: Name, value: impl InternInto<MonomorphicType>) -> Self {
        self.assumptions
    ...
    fn with_constraints(mut self, iter: impl IntoIterator<Item = Constraint>) -> Self {
        self.constraints.extend(iter);
        self
    }

    fn with_assumptions(mut self, set: AssumptionSet<Name>) -> Self {
...
trait Infer<Item, Name> {
    type Error;

    async fn partial_infer<'a>(
        &'a mut self,
        value: &'a Item,
...
mod tests {
    ...
    use crate::process_async::{Infer, InferError, PartialInfer};
