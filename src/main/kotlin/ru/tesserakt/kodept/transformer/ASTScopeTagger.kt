...
class ASTScopeTagger : Transformer() {
    ...
    override fun visit(node: AST.ModuleDecl) = scope({ Scope.Global(node.name) }) {
        node.copy(metadata = node.appendMetadata(currentScope.wrap()),
            rest = node.rest.map { it.acceptTransform(self) })
    }

    override fun visit(node: AST.WhileExpr) = node.copy(
        metadata = node.appendMetadata(currentScope.wrap()),
    ...
    override fun visit(node: AST.CharLiteral) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.BinaryLiteral) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.DecimalLiteral) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.FloatingLiteral) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.HexLiteral) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.OctalLiteral) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.StringLiteral) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.Assignment) = node.copy(metadata = node.appendMetadata(currentScope.wrap()),
        left = node.left.acceptTransform(self),
        right = node.right.acceptTransform(self))

    override fun visit(node: AST.Binary) = node.copy(metadata = node.appendMetadata(currentScope.wrap()),
        left = node.left.acceptTransform(self),
        right = node.right.acceptTransform(self))

    override fun visit(node: AST.Comparison) = node.copy(metadata = node.appendMetadata(currentScope.wrap()),
        left = node.left.acceptTransform(self),
        right = node.right.acceptTransform(self))

    override fun visit(node: AST.Elvis) = node.copy(metadata = node.appendMetadata(currentScope.wrap()),
        left = node.left.acceptTransform(self),
        right = node.right.acceptTransform(self))

    override fun visit(node: AST.Logical) = node.copy(metadata = node.appendMetadata(currentScope.wrap()),
        left = node.left.acceptTransform(self),
        right = node.right.acceptTransform(self))

    override fun visit(node: AST.Mathematical) = node.copy(metadata = node.appendMetadata(currentScope.wrap()),
        left = node.left.acceptTransform(self),
        right = node.right.acceptTransform(self))

    override fun visit(node: AST.Absolution) =
        node.copy(metadata = node.appendMetadata(currentScope.wrap()), expr = node.expr.acceptTransform(self))
    ...
    override fun visit(node: AST.TermChain) = node.copy(metadata = node.appendMetadata(currentScope.wrap()),
    ...
    override fun visit(node: AST.Reference) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.TypeExpression) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
    override fun visit(node: AST.Parameter) = node.copy(
        metadata = node.appendMetadata(currentScope.wrap()),
    ...
    override fun visit(node: AST.EnumDecl.Entry) = node.copy(metadata = node.appendMetadata(currentScope.wrap()))
