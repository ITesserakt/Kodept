...
object DereferenceTransformer : Transformer<AST.Reference> {
    ...
    /**
     * > This works except for Dereference: we should know type of the left branch to find proper reference for the right branch
     *
     * Every reference is in block: ExpressionList or FunctionDecl or VariableDecl or ModuleDecl
     *
    ...
    context(ReportCollector, Filename) override fun transform(node: AST.Reference) =
        eagerEffect<UnrecoverableError, AST.Node> {
            ...
            val referral = if (node.resolutionContext == null) {
                ...
                val resolved = node.resolutionContext.chain.fold(startMapped) { acc, type ->
